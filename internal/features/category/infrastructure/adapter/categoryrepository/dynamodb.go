	...
	transactItems := []dynamodbclient.TransactWriteItem{
		...
		},
		{
			Put: &dynamodbclient.TransactPut{
				...
				Item: map[string]any{
					"PK":          fmt.Sprintf("USER#%s", userID),
					"SK":          fmt.Sprintf("CATEGORY_NAME#%s#%s", category.Name, string(category.TransactionType)),
	...
	transactItems := []dynamodbclient.TransactWriteItem{
		...
		},
		{
			Delete: &dynamodbclient.TransactDelete{
				...
				Key: map[string]any{
					"PK": pk,
					"SK": fmt.Sprintf("CATEGORY_NAME#%s#%s", current.Name, string(current.TransactionType)),
				},
