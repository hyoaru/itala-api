...
func (i findAccountItem) toDomain() (entities.Account, error) {
	balance, err := valueobjects.NewDecimal(i.Balance.String())
	if err != nil {
		return entities.Account{}, fmt.Errorf("parse balance: %w", err)
	}

	createdAt, err := time.Parse(time.RFC3339Nano, i.CreatedAt)
	if err != nil {
		return entities.Account{}, fmt.Errorf("parse created_at: %w", err)
	}

	updatedAt, err := time.Parse(time.RFC3339Nano, i.UpdatedAt)
	if err != nil {
		return entities.Account{}, fmt.Errorf("parse updated_at: %w", err)
	}
...
func (r *DynamoDBAccountRepository) Find(ctx context.Context, userID string, query port.AccountQuery) (port.AccountPage, error) {
	...
	accounts := make([]entities.Account, 0, len(queryItems))
